...
FireworksFractory::FireworksFractory() {
    pMem = new cv::Mat(cv::Size(WND_WIDTH, WND_HEIGHT), CV_8UC3, cv::Scalar(120, 0, 120));
    pMem_ori = new cv::Mat(cv::Size(WND_WIDTH, WND_HEIGHT), CV_8UC3, cv::Scalar(120, 0, 120));
...
int FireworksFractory::getCurrentState(high_resolution_clock::time_point &current_frame_start_time) {
    if (current_frame_start_time - last_shoot_generate_time > milliseconds(1000))   //100ms产生一个烟花弹
    {
        ...
        if (n < total_fire_shoot_num && !shoots[n].show && !flowers[n].show) {
            ...
            shoots[n].resetShoot(max_height, x, y);
        }
    ...
    for (int i = 0; i < total_fire_shoot_num; i++) {
        if (current_frame_start_time - flowers[i].t_now > flowers[i].dt && flowers[i].show) {
            if (flowers[i].cur_r < flowers[i].max_r) {
                flowers[i].cur_r++;
                flowers[i].dt = milliseconds(drt[flowers[i].cur_r / 16]/5);
                flowers[i].show = true;
            }
...
int FireworksFractory::generateCurrentImage() {
    for (int i = 0; i < total_fire_shoot_num; i++) {
        if (shoots[i].show) {
